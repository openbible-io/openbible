...
import { debugPrint, OpLog } from "./egwalker/oplog";
...
export class GenericList<T, AccT extends Accumulator<T>> extends EventTarget {
	...
	protected append(pos: number, items: AccT, updateSnapshot = true): void {
	...
	items(): AccT | undefined {
		let res: AccT | undefined;
		for (const item of this.snapshot.items()) {
			// @ts-ignore idk and idc
			if (!res) res = item;
			// @ts-ignore idk and idc
			else res = this.oplog.mergeFn(res, [item]);
		}
		return res;
	}
	...
	merge(other: GenericList<T, AccT>) {
		...
		console.log("checkout", this.oplog.frontier, this.snapshot);
